...
import type { JellyfinItem, JellyfinLatestItem, JellyfinLibraryView, JellyfinResumeItem } from '@/models/jellyfin.types';
...
const collectionSegments: ReadonlyArray<{
  ...
  { key: 'movies', label: 'Movies', types: ['movies', 'unknown', 'folders'], includeItemTypes: ['Movie'], mediaTypes: ['Video'] },
  { key: 'tv', label: 'TV Shows', types: ['tvshows', 'series'], includeItemTypes: ['Series'], mediaTypes: ['Video'] },
  { key: 'music', label: 'Music', types: ['music'], includeItemTypes: ['Audio'], mediaTypes: ['Audio'] },
];
...
const deriveSubtitle = (item: JellyfinItem, segment: CollectionSegmentKey): string | undefined => {
  if (segment === 'movies') {
...
const JellyfinLibraryScreen = () => {
  ...
  const [activeSegment, setActiveSegment] = useState<CollectionSegmentKey>('movies');
  ...
  const groupedLibraries = useMemo(() => {
    ...
    (librariesQuery.data ?? []).forEach((library) => {
      ...
      const segment = collectionSegments.find((candidate) => candidate.types.includes(type));
      if (segment) {
  ...
  useEffect(() => {
    const firstAvailable = collectionSegments.find((segment) => groupedLibraries[segment.key].length > 0);
    ...
    if (groupedLibraries[activeSegment].length > 0) {
      const libraryIds = groupedLibraries[activeSegment].map((library) => library.Id);
  ...
  const activeSegmentConfig = useMemo(
    () => collectionSegments.find((segment) => segment.key === activeSegment) ?? collectionSegments[0]!,
    [activeSegment],
  );

  const resumeQuery = useJellyfinResume({ serviceId, limit: 12 });
  const latestQuery = useJellyfinLatestItems({ serviceId, libraryId: selectedLibraryId ?? undefined, limit: 16 });
  ...
  const isInitialLoad =
    isBootstrapping || librariesQuery.isLoading || libraryItemsQuery.isLoading || resumeQuery.isLoading || latestQuery.isLoading;
  const isRefreshing =
    libraryItemsQuery.isFetching || resumeQuery.isFetching || latestQuery.isFetching || librariesQuery.isFetching;

  const aggregatedError = librariesQuery.error ?? libraryItemsQuery.error ?? resumeQuery.error ?? latestQuery.error;
  const errorMessage = aggregatedError instanceof Error ? aggregatedError.message : aggregatedError ? 'Unable to load Jellyfin data.' : null;
  ...
  const handleOpenNowPlaying = useCallback(() => {
    ...
    router.push({ pathname: '/(auth)/jellyfin/[serviceId]/now-playing', params: { serviceId } });
  }, [router, serviceId]);
  ...
  const handleOpenItem = useCallback(
    (itemId: string) => {
      ...
      router.push({ pathname: '/(auth)/jellyfin/[serviceId]/details/[itemId]', params: { serviceId, itemId } });
    },
    [router, serviceId],
  );

  const handleRefresh = useCallback(async () => {
    await Promise.all([librariesQuery.refetch(), libraryItemsQuery.refetch(), resumeQuery.refetch(), latestQuery.refetch()]);
  ...
  const renderResumeItem = useCallback(
    ({ item }: { item: JellyfinResumeItem }) => {
      ...
      const subtitle = item.Type === 'Episode' ? item.Name ?? undefined : deriveSubtitle(item, 'movies');
      ...
      return (
        <View style={styles.resumeCard}>
          <MediaPoster uri={posterUri} size={116} accessibilityLabel={`Continue watching ${title}`} onPress={() => handleOpenItem(item.Id)} />
  ...
  const renderLatestItem = useCallback(
    ({ item }: { item: JellyfinLatestItem }) => {
      ...
      const subtitle = item.Type === 'Episode' ? item.SeriesName : deriveSubtitle(item, activeSegment);
      const status = formatEpisodeLabel(item);

      return (
        <View style={styles.latestCard}>
          <MediaPoster uri={posterUri} size={96} onPress={() => handleOpenItem(item.Id)} />
  ...
  const renderLibraryItem = useCallback(
    ({ item, index }: { item: JellyfinItem; index: number }) => {
      ...
      const positionStyle = index % 2 === 0 ? styles.gridCardLeft : styles.gridCardRight;

      return (
        <View style={[styles.gridCard, positionStyle]}>
          <MediaPoster uri={posterUri} size={168} onPress={() => handleOpenItem(item.Id)} />
          <Text variant="bodyMedium" numberOfLines={2} style={styles.gridTitle}>
            {item.Name ?? 'Untitled'}
          </Text>
          {subtitle ? (
            <Text variant="bodySmall" numberOfLines={1} style={styles.gridSubtitle}>
              {subtitle}
            </Text>
          ) : null}
        </View>
      ...
      <View style={styles.headerContainer}>
        <View style={styles.toolbar}>
          ...
          <View style={styles.toolbarActions}>
            <IconButton icon="play-circle-outline" accessibilityLabel="Open now playing" onPress={handleOpenNowPlaying} />
        ...
        <SegmentedButtons
          value={activeSegment}
          onValueChange={(value) => setActiveSegment(value as CollectionSegmentKey)}
        ...
        <View style={styles.sectionHeader}>
          ...
          <IconButton icon="refresh" size={20} accessibilityLabel="Refresh continue watching" onPress={() => void resumeQuery.refetch()} />
        </View>
        ...
        <View style={styles.sectionHeader}>
          ...
          <IconButton icon="refresh" size={20} accessibilityLabel="Refresh latest additions" onPress={() => void latestQuery.refetch()} />
        </View>
