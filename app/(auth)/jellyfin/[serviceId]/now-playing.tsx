...
const computeProgress = (session: JellyfinSession | undefined): number => {
  ...
  const runtime = session.PlayState.RunTimeTicks ?? session.NowPlayingItem?.RunTimeTicks ?? 0;
...
const JellyfinNowPlayingScreen = () => {
  ...
  const nowPlayingQuery = useJellyfinNowPlaying({ serviceId, refetchInterval: 10_000 });
  ...
  const runtimeTicks = activeSession?.PlayState?.RunTimeTicks ?? item?.RunTimeTicks ?? 0;
  const volumeLevel = activeSession?.PlayState?.VolumeLevel ?? 0;

  const isLoading = isBootstrapping || nowPlayingQuery.isLoading;
  const errorMessage = nowPlayingQuery.error instanceof Error ? nowPlayingQuery.error.message : nowPlayingQuery.error ? 'Unable to load playback status.' : null;

  const posterUri = item && connector ? connector.getImageUrl(item.Id, 'Primary', { tag: item.PrimaryImageTag ?? item.ImageTags?.Primary, width: 720 }) : undefined;
  const backdropTag = item?.BackdropImageTags?.[0] ?? item?.ImageTags?.Backdrop;
  const backdropUri = item && connector && backdropTag ? connector.getImageUrl(item.Id, 'Backdrop', { tag: backdropTag, width: 1280 }) : undefined;
  ...
  const handleTogglePlay = useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to toggle playback.';
  ...
  const handleSkip = useCallback(
    async (direction: 'NextTrack' | 'PreviousTrack') => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unable to change track.';
        setStatusMessage(message);
      }
    },
    [activeSession, connector, ensureControlsReady, nowPlayingQuery],
  ...
  const handleSeekRelative = useCallback(
    async (deltaSeconds: number) => {
      ...
      const target = Math.min(Math.max((activeSession.PlayState?.PositionTicks ?? 0) + deltaTicks, 0), runtimeTicks);

      try {
        await connector.sendPlaystateCommand(activeSession.Id, 'Seek', { seekPositionTicks: Math.round(target) });
        void nowPlayingQuery.refetch();
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Unable to seek to the requested position.';
        setStatusMessage(message);
      }
    },
    [activeSession, connector, ensureControlsReady, nowPlayingQuery, runtimeTicks],
  );
  ...
  const handleAdjustVolume = useCallback(
    async (delta: number) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unable to adjust the volume.';
        setStatusMessage(message);
      }
    },
    [activeSession, connector, ensureControlsReady, nowPlayingQuery, volumeLevel],
  );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {backdropUri ? (
        <Image source={{ uri: backdropUri }} style={[StyleSheet.absoluteFillObject, styles.backdropImage]} cachePolicy="memory-disk" contentFit="cover" />
      ) : null}
      {backdropUri ? <View style={[StyleSheet.absoluteFillObject, styles.backdropOverlay]} /> : null}
      <View style={styles.content}>
        <View style={styles.headerRow}>
          <IconButton icon="close" accessibilityLabel="Close now playing" onPress={handleNavigateBack} />
          <Text variant="titleLarge" style={styles.headerTitle}>
            Now Playing
          </Text>
          <IconButton icon="refresh" accessibilityLabel="Refresh status" onPress={handleReload} />
        </View>
        <View style={styles.body}>
          ...
          <View style={styles.controlsRow}>
            <IconButton icon="skip-previous" onPress={() => void handleSkip('PreviousTrack')} />
            <IconButton icon="rewind-10" onPress={() => void handleSeekRelative(-10)} />
            <Button mode="contained" icon={activeSession.PlayState?.IsPaused ? 'play' : 'pause'} onPress={handleTogglePlay}>
              {activeSession.PlayState?.IsPaused ? 'Play' : 'Pause'}
            </Button>
            <IconButton icon="fast-forward-10" onPress={() => void handleSeekRelative(10)} />
            <IconButton icon="skip-next" onPress={() => void handleSkip('NextTrack')} />
          </View>
          <View style={styles.volumeRow}>
            <IconButton icon="volume-minus" onPress={() => void handleAdjustVolume(-5)} />
            <View style={styles.volumeRail}>
              <View style={[styles.volumeFill, { width: `${Math.round(volumeLevel)}%` }]} />
            </View>
            <IconButton icon="volume-plus" onPress={() => void handleAdjustVolume(5)} />
          </View>
        </View>
      </View>
