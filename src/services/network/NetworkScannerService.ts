...
export const SERVICE_DETECTION_CONFIGS: Partial<
  Record<ServiceType, ServiceDetectionConfig>
> = {
  sonarr: {
    type: "sonarr",
    commonPorts: [8989],
...
export class NetworkScannerService {
  ...
  async scanNetwork(
    ...
    try {
      ...
      if (customIpAddress) {
        ...
        totalHosts =
          allHosts.length * Object.keys(SERVICE_DETECTION_CONFIGS).length;
      } else {
      ...
      const priorityServices = await this.scanPriorityHosts(
        priorityHosts,
        progressCallback,
        totalHosts,
        discoveredServices,
        cumulativeScannedHosts,
      );
      discoveredServices.push(...priorityServices);
      cumulativeScannedHosts +=
        priorityHosts.length * Object.keys(SERVICE_DETECTION_CONFIGS).length;
      ...
      if (
        ...
        for (const [serviceType, config] of Object.entries(
          SERVICE_DETECTION_CONFIGS,
        )) {
          ...
          const services = await this.scanServiceType(
            serviceTypeKey,
            config,
            subnet,
            cumulativeScannedHosts,
      ...
      void logger.info("Network scan completed", {
        ...
        scannedHosts: cumulativeScannedHosts,
        subnet,
        servicesFound: discoveredServices.map((s) => ({
          type: s.type,
          url: s.url,
          name: s.name,
        })),
      ...
      for (let i = 0; i < hosts.length; i += CONCURRENT_REQUESTS) {
        ...
        const batchPromises = batch.map(async (host, batchIndex) => {
          ...
          try {
            ...
            const service = await this.detectServiceAtUrl(
              serviceType,
              url,
              config,
            );
        ...
        if (progressCallback && totalHosts && allDiscoveredServices) {
          const currentHostIndex =
            startScannedHosts + Math.min(i + CONCURRENT_REQUESTS, hosts.length);
          progressCallback({
            currentHost: currentHostIndex,
            totalHosts,
            currentService: serviceType,
        ...
        case "qbittorrent":
          ...
          if (typeof response.data === "string") {
            ...
            if (versionMatch) {
              ...
              version = response.data.includes("<")
                ? undefined
                : response.data.trim();
            }
          } else {
  ...
  private async getLocalIpAddress(): Promise<string> {
    try {
      ...
      if (
        ...
        typeof netInfo.details.ipAddress === "string"
      ) {
  ...
  private async scanPriorityHosts(
    ...
    for (const host of priorityHosts) {
      for (const [serviceType, config] of Object.entries(
        ...
        for (const port of config.commonPorts) {
          const promise = this.detectServiceAtUrl(
   ...
   */
  async testServiceUrl(url: string): Promise<{
    success: boolean;
    ...
    try {
      ...
      return {
        success: false,
        error: "No supported service detected at this URL",
      };
    } catch (error) {
      return {
        success: false,
