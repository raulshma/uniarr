...
export class AnalyticsService {
  ...
  public async generateAnalyticsSummary(
    ...
    try {
      ...
      const realData = await this.generateRealAnalyticsSummary(start, end, serviceConfigs);
      ...
      console.warn('Failed to generate real analytics data, falling back to stubs:', error);
    }
  ...
  private async generateRealAnalyticsSummary(
    ...
    return {
      ...
      qualityDistribution: await this.generateQualityDistribution(serviceConfigs),
  ...
  private async generateQualityDistribution(
    ...
    const total = qualityProfiles.reduce((sum, profile) => sum + profile.count, 0);
  ...
  private async generateIndexerPerformance(
    ...
    const indexers = [
      ...
      { name: 'The Pirate Bay', queries: 800, grabs: 180, avgResponseTime: 300 },
  ...
  private async generateActivityTimes(): Promise<ActivityTimes[]> {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      const downloads = Math.floor(Math.random() * 15) + (hour >= 18 && hour <= 23 ? 10 : 0);
      const requests = Math.floor(Math.random() * 5) + (hour >= 9 && hour <= 17 ? 3 : 0);
  ...
  public async exportToCSV(summary: AnalyticsSummary): Promise<string> {
    ...
    summary.libraryGrowth.forEach(item => {
      csvLines.push(`Library Growth - Series,${item.date},${item.sonarrSeries}`);
      csvLines.push(`Library Growth - Movies,${item.date},${item.radarrMovies}`);
