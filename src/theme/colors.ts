...
const lightenColor = (color: string, amount: number): string => mixColor(color, '#FFFFFF', amount);
const darkenColor = (color: string, amount: number): string => mixColor(color, '#000000', amount);
...
const ensureDarkSurface = (color?: string, fallback: string = FALLBACK_DARK_SURFACE): string => {
  ...
  return getRelativeLuminance(normalized) > 0.25 ? darkenColor(normalized, 0.35) : normalized;
};

const ensureLightSurface = (color?: string, fallback: string = FALLBACK_LIGHT_SURFACE): string => {
  const normalized = normalizeHex(color);
  if (!normalized) {
    return fallback;
  }

  return getRelativeLuminance(normalized) < 0.6 ? lightenColor(normalized, 0.35) : normalized;
};

const ensureVariantSurface = (surface: string, isDark: boolean, fallback: string): string => {
...
export const generateThemeColors = (
  ...
  const defaultMode = modeOverrides ?? (isDark
    ? {
  ...
  const primaryContainer = isDark ? lightenColor(primary, 0.28) : darkenColor(primary, 0.18);
  const errorContainer = isDark ? lightenColor(error, 0.3) : darkenColor(error, 0.1);
  ...
  const outline = isDark ? lightenColor(surfaceVariant, 0.25) : darkenColor(surfaceVariant, 0.25);
  const outlineVariant = isDark ? lightenColor(surfaceVariant, 0.15) : darkenColor(surfaceVariant, 0.15);

  const inverseSurface = isDark ? lightenColor(surface, 0.3) : darkenColor(surface, 0.3);
  const inverseOnSurface = getAccessibleOnColor(inverseSurface);
  const inversePrimary = isDark ? lightenColor(primary, 0.4) : darkenColor(primary, 0.4);

  const elevationBase = isDark ? lightenColor(surface, 0.05) : darkenColor(surface, 0.05);
