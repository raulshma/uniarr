...
export function validateApiKey(
  ...
  if (
    ...
    if (!/^[a-zA-Z0-9]+$/.test(trimmedKey)) {
      issues.push(
        `${serviceType} API key should only contain letters and numbers`,
      );
    }
  }
  ...
  if (
    ...
    issues.push(
      `${serviceType} uses username/password authentication, not API keys`,
    );
  }
 ...
 */
export function testApiKeyFormat(
  apiKey: string,
  serviceType:
    | "sonarr"
    | "radarr"
