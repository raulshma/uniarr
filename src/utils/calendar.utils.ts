...
export const sortReleasesByDate = (
  releases: MediaRelease[],
): MediaRelease[] => {
...
export const getReleasesForDate = (
  releases: MediaRelease[],
  date: string,
): MediaRelease[] => {
...
export const getReleasesForWeek = (
  releases: MediaRelease[],
  date: string,
): MediaRelease[] => {
  const startOfWeek = getStartOfWeek(date);
...
export const getReleasesForMonth = (
  releases: MediaRelease[],
  date: string,
): MediaRelease[] => {
...
export const validateDateString = (dateString: string): boolean => {
  ...
  return (
    ...
    !isNaN(date.getTime()) &&
    Boolean(dateString.match(/^\d{4}-\d{2}-\d{2}$/))
  );
};
