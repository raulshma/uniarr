...
export const useOfflineSync = (options: OfflineSyncOptions = {}) => {
  ...
  const processQueuedMutations = async () => {
    ...
    try {
      await logger.info('Processing queued mutations after coming back online', {
  ...
  const processMutation = async (queuedMutation: any) => {
    const { id, mutationFn, queryKey, variables, retryCount } = queuedMutation;
