...
export const useJikanAnimeDetails = (malId?: number) => {
  ...
  const recommendationsQuery = useQuery({
    queryKey: [...queryKeys.discover.jikanDetail(numericMalId ?? 0), 'recommendations'],
  ...
  const picturesQuery = useQuery({
    queryKey: [...queryKeys.discover.jikanDetail(numericMalId ?? 0), 'pictures'],
    enabled: Boolean(numericMalId) && animeQuery.isSuccess,
  ...
  const episodesQuery = useQuery({
    queryKey: [...queryKeys.discover.jikanDetail(numericMalId ?? 0), 'episodes'],
    enabled: Boolean(numericMalId) && animeQuery.isSuccess,
  ...
  const statisticsQuery = useQuery({
    queryKey: [...queryKeys.discover.jikanDetail(numericMalId ?? 0), 'statistics'],
    enabled: Boolean(numericMalId) && animeQuery.isSuccess,
  ...
  const streamingQuery = useQuery({
    queryKey: [...queryKeys.discover.jikanDetail(numericMalId ?? 0), 'streaming'],
    enabled: Boolean(numericMalId) && animeQuery.isSuccess,
  ...
  return {
    anime,
    isLoading: animeQuery.isLoading || (animeQuery.isSuccess && (
      recommendationsQuery.isLoading || reviewsQuery.isLoading ||
      picturesQuery.isLoading || episodesQuery.isLoading ||
      statisticsQuery.isLoading || streamingQuery.isLoading
    )),
    isError: animeQuery.isError || recommendationsQuery.isError || reviewsQuery.isError ||
             picturesQuery.isError || episodesQuery.isError || statisticsQuery.isError || streamingQuery.isError,
    error: animeQuery.error || recommendationsQuery.error || reviewsQuery.error ||
           picturesQuery.error || episodesQuery.error || statisticsQuery.error || streamingQuery.error,
