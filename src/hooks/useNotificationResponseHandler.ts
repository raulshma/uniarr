...
import { NOTIFICATION_CATEGORIES, type NotificationCategory } from '@/models/notification.types';
  ...
  useEffect(() => {
    const subscription = Notifications.addNotificationResponseReceivedListener((response) => {
      try {
        ...
        const categoryRaw = typeof data.category === 'string' ? data.category : undefined;
...
const normalizeCategory = (category: string | undefined): NotificationCategory | null => {
  if (!category) {
    return null;
  }
