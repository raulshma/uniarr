...
import type { VoiceShortcut, VoiceAssistantConfig } from '../services/voice/VoiceAssistantService';
...
export interface UseVoiceAssistantReturn {
  ...
  updateShortcut: (id: string, updates: Partial<VoiceShortcut>) => Promise<void>;
...
export const useVoiceAssistant = (): UseVoiceAssistantReturn => {
  ...
  const [config, setConfig] = useState<VoiceAssistantConfig>(voiceService.getConfig());
  ...
  // Add shortcut
  const addShortcut = useCallback(async (shortcut: Omit<VoiceShortcut, 'id'>) => {
    try {
  ...
  // Update shortcut
  const updateShortcut = useCallback(async (id: string, updates: Partial<VoiceShortcut>) => {
    try {
