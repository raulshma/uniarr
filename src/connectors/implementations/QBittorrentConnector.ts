...
export class QBittorrentConnector extends BaseConnector<Torrent> {
  ...
  async initialize(): Promise<void> {
    logger.debug("[QBittorrentConnector] Initializing", {
      serviceId: this.config.id,
    });
    await this.ensureAuthenticated();
    logger.debug("[QBittorrentConnector] Initialization completed", {
      serviceId: this.config.id,
    });
  }

  async getVersion(): Promise<string> {
    logger.debug("[QBittorrentConnector] Getting version", {
      serviceId: this.config.id,
    });
    await this.ensureAuthenticated();

    try {
      logger.debug("[QBittorrentConnector] Making version request", {
        serviceId: this.config.id,
        url: `${this.config.url}${QB_API_PREFIX}/app/version`,
      });
      const response = await this.client.get<string>(
        `${QB_API_PREFIX}/app/version`,
        {
          responseType: "text",
      ...
      const version =
        typeof response.data === "string" ? response.data.trim() : undefined;
      const finalVersion = version && version.length > 0 ? version : "unknown";
      logger.debug("[QBittorrentConnector] Version retrieved", {
        serviceId: this.config.id,
        version: finalVersion,
      });
      return finalVersion;
    } catch (error) {
      logger.error("[QBittorrentConnector] Version request failed", {
        serviceId: this.config.id,
        error,
      });
  ...
  async getTorrents(filters?: {
    ...
    try {
      const response = await this.client.get<QBittorrentTorrent[]>(
        `${QB_API_PREFIX}/torrents/info`,
        {
          params: {
