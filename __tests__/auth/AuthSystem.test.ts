...
describe("Authentication System", () => {
  ...
  describe("AuthProviderFactory", () => {
    ...
    it("should return correct required credentials for each service type", () => {
      expect(AuthProviderFactory.getRequiredCredentials("sonarr")).toEqual([
        "apiKey",
      ]);
      expect(AuthProviderFactory.getRequiredCredentials("radarr")).toEqual([
        "apiKey",
      ]);
      expect(AuthProviderFactory.getRequiredCredentials("jellyseerr")).toEqual([
        "apiKey",
      ]);
      expect(AuthProviderFactory.getRequiredCredentials("qbittorrent")).toEqual(
        ["username", "password"],
      );
      expect(AuthProviderFactory.getRequiredCredentials("prowlarr")).toEqual([
        "apiKey",
      ]);
    });
  });
  ...
  describe("ServiceAuthHelper", () => {
    ...
    it("should validate service configuration correctly", () => {
      ...
      const validation =
        ServiceAuthHelper.validateServiceConfig(validApiKeyConfig);
      ...
      const invalidValidation =
        ServiceAuthHelper.validateServiceConfig(invalidApiKeyConfig);
      expect(invalidValidation.valid).toBe(false);
      expect(invalidValidation.errors).toContain(
        "API key is required for this service type",
      );
      ...
      const jellyseerrValidation = ServiceAuthHelper.validateServiceConfig(
        validJellyseerrConfig,
      );
      ...
      const invalidJellyseerrValidation =
        ServiceAuthHelper.validateServiceConfig(invalidJellyseerrConfig);
      expect(invalidJellyseerrValidation.valid).toBe(false);
      expect(invalidJellyseerrValidation.errors).toContain(
        "API key is required for this service type",
      );
      ...
      const urlValidation =
        ServiceAuthHelper.validateServiceConfig(invalidUrlConfig);
    ...
    it("should handle different service types correctly", () => {
      // API key service
      const apiKeyConfig = createMockServiceConfig("sonarr", {
        apiKey: "test-key",
      });
      ...
      const sessionAuthConfig =
        ServiceAuthHelper.createAuthConfig(sessionConfig);
