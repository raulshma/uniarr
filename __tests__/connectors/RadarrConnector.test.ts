...
describe('RadarrConnector', () => {
  ...
  it('returns mapped movies from getMovies', async () => {
    ...
    const movieResponse = {
      ...
      images: [
        ...
        { coverType: 'fanart', remoteUrl: 'https://cdn.example.com/backdrop.jpg' },
      ],
    ...
    expect(movie).toMatchObject({
      ...
      monitored: true,
      hasFile: false,
      posterUrl: 'http://radarr.local:7878/media/poster.jpg',
  ...
  it('retrieves movie details with getById', async () => {
    const connector = createConnector();
    const response = { id: 55, title: 'Inception', monitored: true, hasFile: true, images: [] };
    ...
    expect(movie).toMatchObject({ id: 55, title: 'Inception', monitored: true });
  });

  it('creates a movie with add', async () => {
    const connector = createConnector();
    const created = { id: 77, title: 'The Batman Dawn', monitored: true, hasFile: false, images: [] };
  ...
  it('updates monitored state via setMonitored', async () => {
    ...
    mockAxiosInstance.put.mockResolvedValueOnce({ data: { ...existing, monitored: false } });
