...
import { SonarrConnector, type SonarrQueueItem } from '@/connectors/implementations/SonarrConnector';
...
describe('SonarrConnector', () => {
        ...
        records: [
          {
            ...
            episode: { id: 11, title: 'Episode 1', seasonNumber: 1, episodeNumber: 1 },
            status: 'downloading',
  ...
  it('propagates ApiError from search failures', async () => {
    ...
    expect(mockedHandleApiError).toHaveBeenCalledWith(underlyingError, expect.objectContaining({
      operation: 'search',
