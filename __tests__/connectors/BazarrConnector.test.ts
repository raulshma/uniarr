...
describe('BazarrConnector', () => {
  ...
  describe('initialize', () => {
    ...
    it('should handle authentication failure during initialization', async () => {
      ...
      jest.spyOn(connector as any, 'ensureAuthenticated').mockRejectedValue(new Error('Auth failed'));
  ...
  describe('getVersion', () => {
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(connector.getVersion()).rejects.toThrow('Failed to get Bazarr version');
    });
  });
  ...
  describe('getMovies', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(connector.getMovies()).rejects.toThrow('Failed to get movies');
    });
  });
  ...
  describe('getStatistics', () => {
    it('should return statistics when API calls succeed', async () => {
      ...
      jest.spyOn(connector, 'getMovies').mockResolvedValue([
        { id: 1, title: 'Movie 1', missingSubtitles: [{ id: 1, language: { code2: 'en', name: 'English' } }] },
      ]);
      jest.spyOn(connector, 'getEpisodes').mockResolvedValue([
        { id: 1, title: 'Episode 1', missingSubtitles: [{ id: 2, language: { code2: 'es', name: 'Spanish' } }] },
      ]);
      jest.spyOn(connector, 'getSubtitles').mockResolvedValue([
    ...
    it('should handle errors in statistics calculation', async () => {
      ...
      jest.spyOn(connector, 'getMovies').mockRejectedValue(new Error('API error'));

      await expect(connector.getStatistics()).rejects.toThrow('Failed to get statistics');
    });
  });
